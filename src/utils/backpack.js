...
function backpackCanvas(capacity, items, heightValues, { step: number} = {step: 1}){
...
export function backPack2D(
    canvasDimentions = DEFAULT_CANVAS_DIMENTIONS,
    items = DEFAULT_ITEMS.sort((a,b) => a.h -b.h).reverse(),
    solutions = [],
    heightRemaining = DEFAULT_CANVAS_DIMENTIONS.h) {
    const unassignedItems = [];
    let widthRemaining = canvasDimentions.w;
    const rowLevel = canvasDimentions.h-heightRemaining;

    for (let i = 0; i<items.length; i++) {
        if (i == 0){ //0 index is highest value, take first position on this "shelf's row"
            solutions.push({x: 0, y: rowLevel, ...items[i]})
            widthRemaining-=items[i].w;
            heightRemaining-=items[i].h;
        } else {
            if (items[i].w <= widthRemaining){
                solutions.push({x: canvasDimentions.w-widthRemaining, y: rowLevel, ...items[i]})
                widthRemaining-=items[i].w
            }else {
                unassignedItems.push(items[i])
            }
            // Does any other item fit in the remaining row space?
        }
    }
    if (unassignedItems.length > 0){
        backPack2D({...canvasDimentions}, unassignedItems, solutions, heightRemaining)
    }
    return solutions
}
